...
def build_notebook() -> nbf.NotebookNode:
    ...
    nb["cells"] = [
        md(
            ...
            "Quality scope is exactly 1391 test videos and 6955 caption queries. "
            "The source has no caption provenance, therefore caption_source is unknown. "
            "No synthetic fallback is used."
        ),
        code(
            "%pip install -q 'transformers>=4.57.3' 'accelerate>=1.12.0' "
            "'qwen-vl-utils>=0.0.14' huggingface-hub pyarrow pandas numpy\n"
            "!test -d /content/Multimodal-Video-Intelligence || git clone --depth 1 "
            "https://github.com/ColdVI/Multimodal-Video-Intelligence.git /content/Multimodal-Video-Intelligence\n"
            "!test -d /content/Qwen3-VL-Embedding || git clone --depth 1 "
            "https://github.com/QwenLM/Qwen3-VL-Embedding.git /content/Qwen3-VL-Embedding\n"
            "import sys\n"
        ...
        code(
            ...
            "model = Qwen3VLEmbedder(model_name_or_path=MODEL_PATH, fps=1.0, "
            "max_frames=8, max_length=16384, torch_dtype=TORCH_DTYPE, "
        ...
        code(
            ...
            "VIDEO_ROOT = next((p for p in (extract_root/'ERA_Dataset'/'Videos', "
            "extract_root/'Videos') if p.exists()), None)\n"
        ...
        code(
            ...
            "cfg['datasets']['capera'].update({'train_split': str(TRAIN_JSON), "
            "'test_split': str(TEST_JSON), 'videos_dir': str(VIDEO_ROOT)})\n"
            ...
            "    items.append({'segment_id': segment_id, 'video_id': video_id, "
            "'video_path': str(video_path)})\n"
            "    for caption_index, text in enumerate(captions):\n"
            "        queries.append({'query_id': f'capera:{video_id}:caption:{caption_index}', "
            "'query_text': text, 'relevant_segment_id': segment_id, "
            "'relevant_video_id': video_id, 'caption_index': caption_index, "
        ...
        code(
            ...
            "    else:\n"
            "        matrix = np.lib.format.open_memmap(partial, mode='w+', "
            "dtype=np.float32, shape=(count, 2048)); matrix[:] = np.nan\n"
        ...
        code(
            "query_partial, query_done_path, query_vectors, query_done = resume("
            "'capera_queries_2048', 6955)\n"
        ...
        code(
            "semantic = json.loads(Path('/content/Multimodal-Video-Intelligence/tests/fixtures/"
            "queries_semantic.json').read_text(encoding='utf-8'))\n"
            "demo_texts = []\n"
            "for row in semantic:\n"
            "    if 'generator' in row:\n"
            ...
            "fixed = {text: normalize(vector.detach().cpu().float().numpy()).tolist() "
            "for text, vector in zip(demo_texts, outputs)}\n"
            "(OUT/'query_embeddings.json').write_text(json.dumps({'model_id': MODEL_ID, "
            "'model_revision': MODEL_REVISION, 'queries': fixed}, ensure_ascii=False), "
            "encoding='utf-8')\n"
        ...
        code(
            ...
            "manifest = {'dataset_id':'capera','split':'test','item_count':1391,"
            "'query_count':6955,'captions_per_item':5,'caption_source':'unknown',"
            "'dimension':2048,'model_id':MODEL_ID,'model_revision':MODEL_REVISION,"
            "'dtype':str(TORCH_DTYPE),'attention_implementation':ATTN_IMPL,'gpu_name':GPU_NAME,"
            "'frames_per_item':8,'embedding_mode':'real','item_elapsed_s':ITEM_ELAPSED_S,"
            "'query_elapsed_s':QUERY_ELAPSED_S,"
            "'test_caption_sha256':hashlib.sha256(TEST_JSON.read_bytes()).hexdigest()}\n"
            "(OUT/'embedding_manifest.json').write_text(json.dumps(manifest,indent=2),encoding='utf-8')\n"
            "print(manifest)"
        ),
        code(
            "from google.colab import files\n"
            "names = ['capera_2048.npy','capera_ids.parquet','capera_queries_2048.npy',"
            "'capera_query_ids.parquet','query_embeddings.json','embedding_manifest.json']\n"
...
if __name__ == "__main__":
    target = Path(__file__).with_name("07_colab_embedding_production.ipynb")
