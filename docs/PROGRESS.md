...
| 2026-07-30 | Aşama 4 model bundle/Qwen | Qwen kaynak/model revision'ları sabitlendi; hash-verified bundle hazırlama, download-free CUDA image, read-only GPU Compose mount, tek-call batch API ve GPU smoke runner eklendi. | Bundle/batch/CPU fail-closed testleri 9/9 PASS; GPU Compose parse PASS; `artifacts/faz11/gpu_smoke.json` bu macOS/arm64 CUDA'sız hostta dürüst `not_run`. |
| 2026-07-30 | Aşama 5 run-versioned storage | Additive control-plane/run metadata tabloları, backend run anahtarları, chunk retry/finalize/activation, request-wide active snapshot, deterministic legacy migration planı ve retention güvenlikli GC eklendi. | Recovery/snapshot/profile testleri 24/24 PASS. `artifacts/faz11/schema_migration_report.json` host test Python'ında psycopg2 wheel/çalışan DB olmadığı için `not_run`; hiçbir apply/destructive işlem yapılmadı. |
| 2026-07-30 | Aşama 6 generic real ingest | Manifest CLI, streaming Qwen batch→enabled dimension/backend writes, run metadata, chunk resume, count/finalize, run report/error ve legacy CLI compatibility eklendi. | Streaming+crash/resume/run testleri 17/17 PASS; `artifacts/faz11/ingest_resume_smoke.json` gerçek GPU/data/DB olmadığı için `not_run`, fake-store contract kanıtı ayrıca PASS. |
| 2026-07-30 | Aşama 7 native filter pushdown | Run-scoped filter registry, canonical ClickHouse/Qdrant projections, pgvector JOIN compiler, circular filters, active-run native engine, adaptive predicate reuse, legacy cap ve exact equivalence runner eklendi. | Compiler/engine/ingest/run odak paketi 21/21 PASS. `filter_equivalence.json` ve `search_scale_smoke.json` canlı active corpus olmadığı için `not_run`; ölçüm sayısı uydurulmadı. |
| 2026-07-30 | Aşama 8 UI/media | Additive dataset/run/filter-schema/media API'leri, active-run stats, API-driven backend/dimension seçenekleri, registry-controlled canonical filtreler, run/provenance/model ve read-only extra telemetry detail görünümü, güvenli H.264 cache eklendi. | Service suite `99 passed, 17 skipped`; medya/UI odak paketi dahil. Canlı UI/MP4 kabulü `artifacts/faz11/ui_smoke.{png,json}` içinde dürüst `not_run`. |
| 2026-07-30 | Aşama 9 security/docs/operations | Optional Bearer middleware, token'sız loopback compatibility, expiring signed media URL, public-bind preflight guard, env-driven Compose memory ayarları ve kurum deployment/operations/migration/rollback/troubleshooting rehberleri eklendi. | Auth/profile/media/preflight odak testleri ve dört Compose birleşimi PASS; canlı deployment Stage 10 ortam kapısına bırakıldı. |
| 2026-07-30 | Aşama 10 final evidence | Bütün pure testler, syntax, diff ve dört Compose config yeniden çalıştırıldı; mandatory artifact seti ve final acceptance/report üretildi. | Root `314 passed, 42 skipped`; service `104 passed, 17 skipped`; 19 PASS, 0 FAIL, 0 BLOCKED, 9 NOT RUN; final durum `implementation_complete_hardware_acceptance_pending`. |
| 2026-07-31 | Bağımsız denetim (farklı host, Windows) | `docs/FAZ11_FINAL_REPORT.md` kaynak gerçek kabul edilmeden kod yeniden okundu ve testler farklı bir makinede yeniden çalıştırıldı. İki gerçek defekt bulundu ve düzeltildi: (F1) `DECODE_PREFETCH_WINDOWS`/`DB_WRITE_BATCH_SIZE` ölü konfigürasyondu, gerçek ingest döngüsü üç ayarı da `EMBED_BATCH_SIZE`'a indirgemişti; ingest.py üç bağımsız katmana (decode prefetch / embed batch / DB write batch) ayrıldı. (F2) `iter_chunk_windows` kaynak frame'i ve `GeneratorExit` durumunu açıkça kapatmıyordu; `try/finally` ownership eklendi. | `docs/FAZ11_TRACEABILITY_AUDIT.md`, `artifacts/faz11/traceability_audit.json`; service `113 passed, 15 skipped` (7 yeni test), root `356 passed, 0 skipped`, py_compile/diff-check PASS. |
