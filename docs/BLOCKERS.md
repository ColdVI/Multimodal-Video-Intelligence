...
- 2026-07-30 — `nbformat` host Python ortamında kurulu değil; notebook JSON olarak parse edildi ve tüm kod hücreleri AST kontrolünden geçti; bağımsız sıradaki adım: Docker build sırasında pinli `nbformat==5.10.4` ile yeniden validate etmek.
- 2026-07-30 — UI shell gerçek Gradio sürecinde HTTP 200 verdi ve Chrome ile PNG alındı; API/DB kapalı olduğundan ekran görüntüsünde dataset/facet verisi boş; sıradaki adım: Docker Engine açıldıktan ve AU-AIR yüklendikten sonra aynı RUNBOOK adımlarıyla dolu sonuç ekranı çekmek.
